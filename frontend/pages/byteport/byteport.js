import PermissionMenu from '/byteport/permission_menu.js';
...
async function loadResources() {
    ...
    // Send request
    const res = await fetch(`/api/byteport/${folder_path_tail.folder_id}`);
    ...
    const byteport_path = document.querySelector('.byteport_path');
    byteport_path.innerHTML = '';

    const byteport_path_label = document.createElement('p');
    byteport_path_label.classList.add('byteport_path_label');
    byteport_path_label.innerText = 'Current folder:';

    const byteport_path_elements = (() => {
        let elements = [];

        // For sub-directories
        for (let i = 0; i < folder_path.length; i++) {
            const byteport_path_element = document.createElement('span');
            byteport_path_element.classList.add('byteport_path_element');
            byteport_path_element.innerText = `${folder_path[i].folder_name}/`;

            byteport_path_element.addEventListener('click', () => {
                // Slice the folder_path array to include 0 up to the i-th element (inclusive)
                folder_path = folder_path.slice(0, i + 1);
                loadResources();
            });

            elements.push(byteport_path_element);
        }
    ...
    // Add all elements to byteport_path div
    byteport_path.append(byteport_path_label);
    byteport_path_elements.forEach(byteport_path_element => {
        byteport_path.append(byteport_path_element);
    });
...
async function downloadFile(file_id) {
    ...
    window.location.href = `/api/byteport/download/${file_id}`;
}
...
async function uploadFile(file) {
    ...
    const res = await fetch(`/api/byteport/upload/${currentFolderId}`, {
        method: 'POST',
        body: formData
    });
...
async function deleteFile(file_id) {
    if (confirm("Are you sure you want to delete this file?")) {
        const res = await fetch(`/api/byteport/delete`, {
            method: 'DELETE',
            headers: {
...
async function moveFile(file_id, new_parent_folder) {
    const res = await fetch(`/api/byteport/move`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
...
document.querySelector('#new_folder').addEventListener('click', async (event) => {
    ...
    const res = await fetch(`/api/byteport/folder`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
